...
# recurion and death. These two events will be modelled based on age and disease
# stage as covariates. The probability of the event being recurion and the scale
# parameter for the Weibull time-to-recurion distribution will be modelled based 
...
source(file = 'ready_made_function/ESPD_frequentist.R')
...
simESPD <- function(pars, n_sim = 500, seed = 123) {
  ...
  df[ , `:=` (
    prob1  = getProb(X_stage %*% pars$recur_logitTheta),
    shape1 = exp(pars$recur_logShape),
    scale1 = exp(X_stage %*% pars$recur_logScale),
  ...
  df[ , `:=` (
    event = fifelse(runif(.N) < prob1, 'recur', 'death'),
    time1 = rweibull(.N, shape1, scale1),
    time2 = rgompertz(.N, shape2, rate2)
  )]
  df[ , time := fifelse(event=='recur', time1, time2)]
...
pars <- list(
  recur_logitTheta = c('Intercept' = -0.4, 'IB' = 0.4, 'II' = 0.8),
  recur_logShape   = c('Intercept' = 0.7),
  recur_logScale   = c('Intercept' = 2, 'IB' = -0.2, 'II' = -0.6),
...
# Naive illustration
fitESPD(t = ESPD_0$time, e = ESPD_0$event, events = c('recur', 'death'), dist1 = 'weibull', dist2 = 'gompertz')

# Fitting ESPD parameters
fit_0  <- fitESPD(t = ESPD_0$time,  e = ESPD_0$event,  X1 = ESPD_0$X_stage,  X12 = ESPD_0$X_stage,  X22 = ESPD_0$X_age,  events = c('recur', 'death'), dist1 = 'weibull', dist2 = 'gompertz')
fit_10 <- fitESPD(t = ESPD_10$time, e = ESPD_10$event, X1 = ESPD_10$X_stage, X12 = ESPD_10$X_stage, X22 = ESPD_10$X_age, events = c('recur', 'death'), dist1 = 'weibull', dist2 = 'gompertz')
fit_30 <- fitESPD(t = ESPD_30$time, e = ESPD_30$event, X1 = ESPD_30$X_stage, X12 = ESPD_30$X_stage, X22 = ESPD_30$X_age, events = c('recur', 'death'), dist1 = 'weibull', dist2 = 'gompertz')
fit_60 <- fitESPD(t = ESPD_60$time, e = ESPD_60$event, X1 = ESPD_60$X_stage, X12 = ESPD_60$X_stage, X22 = ESPD_60$X_age, events = c('recur', 'death'), dist1 = 'weibull', dist2 = 'gompertz')

# Comparison of coefficients
rbind(
  truth  = c(pars$recur_logitTheta, pars$recur_logShape, pars$recur_logScale, pars$death_Shape, pars$death_logRate),
...
# Probability of recurion
rbind(
  truth  = getProb(cumsum(pars$recur_logitTheta)),
...
# Shape parameter for recurion
rbind(
  truth  = exp(pars$recur_logShape),
...
# Scale parameter for recurion
rbind(
  truth  = exp(cumsum(pars$recur_logScale)),
...
getOutcomes <- function(t, e, TTP_min, TTP_max, TTD_min, TTD_max) {
  ...
  out <- list(
    prob_TTP = mean(e == 'recur'),
    mean_TTP = mean(t[e == 'recur']),
    mean_TTD = mean(t[e == 'death']),
    dens_TTP = density(x = t[e == 'recur'], n = 1024, from = TTP_min, to = TTP_max),
...
runSIM <- function(pars, p_censoring, n_sample, n_runs = 1000, n_sim = 10^5, n_cores = 12, seed = 123) {
  ...
  TTP_min  <- TTD_min <- 0
  TTP_max  <- max(ESPD_pop$time[ESPD_pop$event == 'recur'])
      ...
      out_run <- parSapply(cl, 1:n_runs, function(i_run) {
        ...
        while(is.null(out_run)) {
          ...
          out_run <- tryCatch(expr = {
            ...
            fit_pars <- list(
              recur_logitTheta = fit$estimates[1:3],
              recur_logShape   = fit$estimates[4],
              recur_logScale   = fit$estimates[5:7], 
